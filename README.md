...
The bridge sends a page-independent `bridge-ready` acknowledgement when it starts, then reports only `unknown`, `auth-required`, or `authenticated` for the visible portal state. It does **not** read browser cookies, storage, passwords, tokens, authorization headers, or network bodies. The core does **not** establish a VPN tunnel, modify routes or DNS, or replace an official client. Its tray state deliberately says that further connection capability is pending. See [the browser bridge note](./docs/BrowserAuthBridge.zh-CN.md) and [the Chinese CquVpnCore note](./docs/CquVpnCore.zh-CN.md) for the precise boundary.
